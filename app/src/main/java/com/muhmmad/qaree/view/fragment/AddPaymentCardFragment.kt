...
import com.muhmmad.qaree.viewModel.AddPaymentCardViewModel
...
class AddPaymentCardFragment : Fragment() {
    ...
    private fun checkValidation(
        ...
        if (name.isEmpty()) {
            binding.layoutName.error = getString(com.muhmmad.qaree.R.string.enter_card_holder_name)
            return false
        } else if (name.length < 6) {
            binding.layoutName.error = getString(com.muhmmad.qaree.R.string.enter_valid_card_holder_name)
            return false
        } else binding.layoutName.error = null

        if (cardNumber.isEmpty()) {
            binding.layoutCardNumber.error = getString(com.muhmmad.qaree.R.string.enter_card_number)
            return false
        } else if (cardNumber.length != 16) {
            binding.layoutCardNumber.error = getString(com.muhmmad.qaree.R.string.enter_valid_card_number)
            return false
        } else binding.layoutCardNumber.error = null

        val currentDate = LocalDate.now().plusMonths(1)
        if (expireData.isBefore(currentDate)) {
            activity.showError(binding.root, getString(com.muhmmad.qaree.R.string.your_card_is_expired))
            return false
        }

        if (cvv.isEmpty()) {
            binding.layoutCvv.error = getString(com.muhmmad.qaree.R.string.enter_cvv)
            return false
        } else if (cvv.length != 3) {
            binding.layoutCvv.error = getString(com.muhmmad.qaree.R.string.enter_valid_cvv)
            return false
